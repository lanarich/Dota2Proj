...
def predict_winner(match_id):  # тут он должен принимать уже датафрейм
    ...
    try:
        ...
        print(end, "| info | конец | endpoint: {predict_completed} | предсказание победы | match_id : "
              + str(match_id))
...
async def get_info(match_id: int, background_tasks: BackgroundTasks):
    start = datetime.datetime.now()
    print(start, "| info | начало | endpoint: {get_match_info} | информация о матче | match_id : "
          + str(match_id))
    try:
        ...
        print(end, "| info | конец | endpoint: {get_match_info} | информация о матче | match_id : "
              + str(match_id))

        return FileResponse(path=filename, filename=filename, media_type='application/json')
    except Exception as e:
        end = datetime.datetime.now()
        print(end, "| error | конец | endpoint: {get_match_info} | информация о матче | match_id : "
              + str(match_id) + str(e))
        raise HTTPException(status_code=404, detail="Troubles with getting match info")
...
def get_info(match_id):
    start = datetime.datetime.now()
    print(start, "| info | начало | endpoint: {get_match_dataframe} | датафрейм по матчу | match_id : "
          + str(match_id))
    try:
        converted_match = get_decomposed_match_data_wout_db(match_id)
        end = datetime.datetime.now()
        print(end, "| info | конец | endpoint: {get_match_dataframe} | датафрейм по матчу | match_id : "
              + str(match_id))

        return HTMLResponse(content=converted_match.to_html())
    except Exception as e:
        end = datetime.datetime.now()
        print(end, "| error | конец | endpoint: {get_match_dataframe} | датафрейм по матчу | match_id : "
              + str(match_id) + str(e))
        raise HTTPException(status_code=404, detail="Troubles with converting data to dataframe")


@app.get('/gold_graph/{match_id}')
def get_gold_graph(match_id):  # тут он должен принимать уже датафрейм
    start = datetime.datetime.now()
    print(start, "| info | начало | endpoint: {gold_graph} | график золота по матчу | match_id : "
          + str(match_id))
    try:
        ...
        print(end, "| info | конец | endpoint: {gold_graph} | график золота по матчу | match_id : "
              + str(match_id))

        return StreamingResponse(buf, media_type="image/png")
    except Exception as e:
        end = datetime.datetime.now()
        print(end, "| error | конец | endpoint: {gold_graph} | график золота по матчу | match_id : "
              + str(match_id) + str(e))
        raise HTTPException(status_code=404, detail="Troubles with getting match info")
...
def get_exp_graph(match_id):  # тут он должен принимать уже датафрейм
    start = datetime.datetime.now()
    print(start, "| info | начало | endpoint: {exp_graph} | график опыта по матчу | match_id : "
          + str(match_id))
    try:
        game = get_decomposed_match_data_wout_db(match_id)
        game = game.sort_values('currentMinute').reset_index(drop=True)
        ...
        print(end, "| info | конец | endpoint: {exp_graph} | график опыта по матчу | match_id : "
              + str(match_id))
        return StreamingResponse(buf, media_type="image/png")
    except Exception as e:
        end = datetime.datetime.now()
        print(end, "| error | конец | endpoint: {exp_graph} | график опыта по матчу | match_id : "
              + str(match_id) + str(e))
        raise HTTPException(status_code=404, detail="Troubles with getting match info")
