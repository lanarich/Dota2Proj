...
def convert_match_data_into_df(current_match_data):
    ...
    try:
        ...
        radiantNetworthLeads = get_collection_attr(
            current_match_data, 'radiantNetworthLeads')

        # читаем постройки и заполняем единицами, потому что сначала они все
        # целы
        ...
        for minute in range(len(radiantNetworthLeads[2:])):
            ...
            minuteRadiantExperienceLeads = get_collection_attr(
                current_match_data, 'radiantExperienceLeads')[minute]
            ...
            minuteRadiantKills = get_collection_attr(
                current_match_data, 'radiantKills')[minute]
            #  количество убийств свет
            obj_dict['minuteRadiantKills'] = minuteRadiantKills
            minuteDireKills = get_collection_attr(
                current_match_data, 'direKills')[minute]
            ...
            get_tower_barracks_destroy_dict(
                get_collection_attr(
                    current_match_data,
                    'towerDeaths'),
                minute,
            ...
            # result = dataframe_match_collection.update_one(obj_dict, update,
            # upsert=True)
...
def single_player_data(player_dict, minute, player, side, pos):
    ...
    # player_dict[side + pos + '_attackType'] = player['hero']['stats']
    # ['attackType']
    # player_dict[side + pos + '_startingArmor'] = player['hero']['stats']
    # ['startingArmor']
    # player_dict[side + pos + '_startingDamageMin'] = player['hero']['stats']
    # ['startingDamageMin']
    # player_dict[side + pos + '_startingDamageMax'] = player['hero']['stats']
    # ['startingDamageMax']
    # player_dict[side + pos + '_attackRate'] = player['hero']['stats']
    # ['attackRate']
    # player_dict[side + pos + '_attackAnimationPoint'] = player['hero']
    # ['stats']['attackAnimationPoint']
    # player_dict[side + pos + '_attackAcquisitionRange'] = player['hero']
    # ['stats']['attackAcquisitionRange']
    # player_dict[side + pos + '_attackRange'] = player['hero']['stats']
    # ['attackRange']
    # player_dict[side + pos + '_primaryAttribute'] = player['hero']['stats']
    # ['primaryAttribute']
    # player_dict[side + pos + '_strengthBase'] = player['hero']['stats']
    # ['strengthBase']
    # player_dict[side + pos + '_strengthGain'] = player['hero']['stats']
    # ['strengthGain']
    # player_dict[side + pos + '_intelligenceBase'] = player['hero']['stats']
    # ['intelligenceBase']
    # player_dict[side + pos + '_intelligenceGain'] = player['hero']['stats']
    # ['intelligenceGain']
    # player_dict[side + pos + '_agilityBase'] = player['hero']['stats']
    # ['agilityBase']
    # player_dict[side + pos + '_agilityGain'] = player['hero']['stats']
    # ['agilityGain']
    # player_dict[side + pos + '_moveSpeed'] = player['hero']['stats']
    # ['moveSpeed']

    # for item in range(6):
    # if player['stats']['inventoryReport'][minute]['item' + str(item)]
    # is not None:
    ...
    # str(backpack_item)] is not None:
    # player_dict[side + pos + '_backpack_' + str(backpack_item)] = \
    # player['stats']['inventoryReport'][minute]['backPack' +
    # str(backpack_item)]['itemId']
    # else:
