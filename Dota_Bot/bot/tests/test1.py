...
from handlers.predict_items import load_model, dataframe_preprocessing, predict_csv_file, columns_for_drop, columns_true_false_convert, columns_with_leaver_status
import pandas as pd
from handlers.user_mark import select_mark
import pytest
...
class TestLoadModel(unittest.TestCase):
    ...
    @patch('builtins.open', new_callable=mock_open, read_data=b'random_forest_model_data')
    @patch('joblib.load')
...
class TestPlaceKb(unittest.TestCase):
    def test_place_kb(self):
        ...
        # Проверяем, что возвращаемый объект является экземпляром InlineKeyboardMarkup
...
class TestDataframePreprocessing(unittest.TestCase):
    ...
    def test_dataframe_preprocessing(self):
        ...
        processed_df = dataframe_preprocessing(df, predict_items=True)
...
class TestPredictCsvFile(unittest.TestCase):
    ...
    def test_predict_csv_file(self, mock_read_csv, mock_dataframe_preprocessing, mock_load):
        ...
        # Проверки
        mock_read_csv.assert_called_once_with(file_name)  # Проверяем, что функция pd.read_csv была вызвана с правильным аргументом
        mock_dataframe_preprocessing.assert_called_once()  # Проверяем, что функция dataframe_preprocessing была вызвана
        mock_model.predict_proba.assert_called_once()  # Проверяем, что метод predict_proba был вызван у модели
        self.assertEqual(predictions, mock_model.predict_proba.return_value[:, 1])  # Проверяем, что возвращаемое значение соответствует предсказаниям модели


class TestDataBase(unittest.TestCase):

    def setUp(self):
        self.db = DataBase(':memory:')  # Используем временную базу данных для тестов

    def test_create_db(self):
        # Проверяем, что таблица users создается успешно
        self.db.create_db()
        self.assertIn(('users',), self.db.cursor.execute("SELECT name FROM sqlite_master WHERE type='table';").fetchall())
    ...
    def test_add_mark(self):
        ...
        result = self.db.cursor.execute("SELECT * FROM users WHERE telegram_id = ?", (telegram_id,)).fetchone()
    ...
    def test_select_marks(self):
        ...
        self.assertEqual(len(marks), 0)  # Предполагаем, что изначально нет меток
        self.db.add_mark('A', '12345')
        marks = self.db.select_marks()
        self.assertEqual(len(marks), 1)  # Проверяем, что после добавления есть одна метка
    ...
    def test_select_user_id(self):
        ...
        self.assertEqual(user[2], telegram_id)  # Проверяем, что telegram_id совпадает
