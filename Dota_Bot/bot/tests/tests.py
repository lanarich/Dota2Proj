...
class TestBot(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_start_command(self, get_started_mock):
        message = types.Message(message_id=1, date=1, chat=types.Chat(id=1, type='private'), text='/start')
    ...
    async def test_help_command(self, get_help_mock):
        message = types.Message(message_id=1, date=1, chat=types.Chat(id=1, type='private'), text='/help')
    ...
    async def test_predict_items_command(self, start_predict_items_mock):
        ...
        await start_predict_items_mock(message, FSMContext(storage=None, key='test_key'))
    ...
    async def test_user_mark_command(self, get_inline_mock):
        message = types.Message(message_id=1, date=1, chat=types.Chat(id=1, type='private'), text='📌 Оценить бота 📌')
        get_inline_mock.return_value = AsyncMock()
        await get_inline_mock(message, FSMContext(storage=None, key='test_key'))
    ...
    async def test_statistics_command(self, statistic_mock):
        message = types.Message(message_id=1, date=1, chat=types.Chat(id=1, type='private'), text='📊 Статистика бота 📊')
