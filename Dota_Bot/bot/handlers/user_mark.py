...
async def get_inline(message: Message):
    ...
    if (users):
        await message.answer('Вы уже оценили сервис. Спасибо!',
                             reply_markup=predict_item_keyboard)
    else:
        await message.answer('Оцените меня:', reply_markup=place_kb())
...
async def select_mark(call: CallbackQuery):
    ...
    if (users):
        await call.message.answer('Вы уже оценили сервис. Спасибо!')
    else:
        answer = 'Ваше мнение важно для нас!'
